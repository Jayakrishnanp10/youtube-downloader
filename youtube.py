...
class youtube(Frame):
    def __init__(self):
        ...
        self.suc=StringVar()
        self.suc.set(" ")
        self.label4=Label(self,textvariable=self.suc,font='Helvetica 7 bold')
        self.label4.grid(row=4,column=0,columnspan=5)
        self.label4.configure(bg="#e85348",fg='white')
        
        
        
    def ydownload(self):
        YouTube(self.url.get()).streams.get_by_resolution(self.reso.get()).download(self.loc.get())
        self.suc.set(self.url.get()+" "+self.reso.get()+" DOWNLOAD COMPLETED")
